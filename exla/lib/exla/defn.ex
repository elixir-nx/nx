defmodule EXLA.Defn do
  ...
  defp to_stream_computation(
    ...
    out_flat = Nx.Defn.Composite.flatten_list([expr])
    ...
    acc_shape = Enum.map(acc_shapes, &elem(&1, 1))

    constant_shape = Enum.map(used_shapes, &elem(&1, 1))
    ...
    pred_fun = EXLA.Builder.new({module, name}, arg_shapes, out_flat, :mlir)
    ...
    body_fun = EXLA.Builder.new({module, name}, arg_shapes, out_flat, :mlir)

    [_flag, token | body_args] = EXLA.MLIR.Function.get_arguments(body_fun)

    {acc, constant} = Enum.split(body_args, acc_length)
    ...
    [_flag, token | results] =
      Value.while(builder, pred, body_fun, [Value.infeed(root_token, flag_shape) | args])

    outfeed = outfeed |> Outfeed.with_token(token) |> Outfeed.close(builder)

    Value.variadic_return(builder, results, true)
    # EXLA.Builder.build(Value.tuple(builder, results))
        ...
        comp_cache_fun.(comp_key, fn ->
          ...
          builder =
            if mode == :mlir do
              arg_shapes =
                inputs_and_shapes
                |> Enum.filter(fn {i, _} -> i >= used_buffers end)
                |> Enum.map(fn {_, shape} ->
                  EXLA.Builder.exla_shape(shape)
                end)

              outputs =
                [outputs]
                |> Nx.Defn.Composite.flatten_list()
                |> Enum.map(&(&1 |> Nx.devectorize() |> EXLA.Builder.exla_shape()))
  ...
  defp op_computation(
    ...
    function =
      EXLA.Builder.new({module, name}, arg_shapes, struct(Nx.Tensor, out), :mlir)
  ...
  defp fun_computation(
    ...
    function = EXLA.Builder.new({module, Atom.to_string(name)}, arg_shapes, expr, :mlir)
  ...
  defp while_computation(name, arg, expr, type, transform, %{builder: %Function{}} = state, cache) do
    ...
    function =
      EXLA.Builder.new({module, name}, arg_shapes, out_expr, :mlir)
    ...
    res =
      if type == :with_token do
        [get_token(comp_cache) | List.wrap(res)]
      else
        List.wrap(to_type(res, type))
      end
  ...
  defp token_computation(name, args, expr, %{builder: %Function{}} = state, cache) do
    arg_shapes =
      Enum.map(args, &Value.get_shape/1)
    ...
    function =
      EXLA.Builder.new({module, name}, [token_shape | arg_shapes], [token_shape, expr], :mlir)
  ...
  defp while_arg_shape(list) when is_list(list) do
    list
    |> Enum.map(&while_arg_shape/1)
    |> List.flatten()
  end
  ...
  defp computation_arg_param({tuple, [%Value{} | _] = param}) when is_tuple(tuple) do
    tuple
    |> Tuple.to_list()
    |> Enum.with_index(fn arg, i -> {arg, Enum.fetch!(param, i)} end)
    ...
    case builder do
      %EXLA.MLIR.Function{} ->
        ...
        if_results =
          Value.if(
            pred_op,
            token_shape ++ List.wrap(EXLA.Builder.exla_shape(on_true)),
            true_args,
            true_comp,
            false_args,
            false_comp
  ...
  defp if_branch_computation(
    ...
    out_flat =
      [out_expr]
      |> Nx.Defn.Composite.flatten_list()
      |> Enum.map(&EXLA.Builder.exla_shape/1)

    if token = get_token(cache) do
      inputs = Enum.map(args, &Value.get_shape/1)
      token_shape = EXLA.Shape.make_token_shape()
      inputs = [token_shape | inputs]
      ...
      function =
        EXLA.Builder.new({module, name}, inputs, [token_shape | out_flat], :mlir)
      ...
      # input function is actually the parent function still, so we need to actually create a new function
      # with this name on the same module.
      function = EXLA.Builder.new({module, name}, inputs, out_flat, :mlir)
