defmodule EXLA.MLIR.Value do
  ...
  def is_infinity(%Value{function: func} = operand, output_typespec) do
    %{type: type} = get_typespec(operand)

    typespec = Typespec.to_type(output_typespec, {:pred, 8})
    ...
    if output_typespec.type == {:u, 8} do
      convert(result, output_typespec)
    else
      result
    end
  end

  def is_nan(%Value{function: func} = operand, output_typespec) do
    %{type: type} = get_typespec(operand)
    %{type: output_type} = output_typespec
