defmodule EXLA.MLIR.Value do
  ...
  def elixir_call([%Value{function: func} | _] = operands, typespecs, callback_server_pid, callback_id) do
    ...
    pid_size = byte_size(pid_bin)

    # Zero-pad the pid binary so its size is a multiple of 8 and it can be
    # represented as a list of 64-bit words.
    pad =
      case rem(pid_size, 8) do
        0 -> 0
        r -> 8 - r
      end
    ...
    padded_bin =
      if pad == 0 do
        ...
        pid_bin <> :binary.copy(<<0>>, pad)
      end

    callback_server_pid_words =
