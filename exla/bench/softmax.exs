...
benches =
  if System.get_env("EXLA_TARGET") == "cuda" do
    ...
    Map.merge(benches, %{
      "xla jit-gpu f32" => {fn -> cuda_jit.(dt32) |> Nx.add(1) end, after_each: &Nx.backend_deallocate/1},
      "xla jit-gpu f64" => {fn -> cuda_jit.(dt64) |> Nx.add(1) end, after_each: &Nx.backend_deallocate/1}
    })
  else
    benches
  end
