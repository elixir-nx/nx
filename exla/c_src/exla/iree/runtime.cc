...
run_module(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  if (!iree_status_is_ok(status)) {
    ...
    std::string status_msg = get_status_message(status);

    return exla::nif::error(env, status_msg.c_str());
  }
...
ERL_NIF_TERM create_instance(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  iree_vm_instance_t *instance = create_instance();

  return instance != nullptr ? exla::nif::ok(env, exla::nif::make<iree_vm_instance_t *>(env, instance)) : exla::nif::error(env, "Failed to create IREE VM instance");
}
...
ERL_NIF_TERM read_buffer_to_term(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  iree_status_t status = read_buffer(*device, *buffer_view, binary.data, num_bytes_actual);
