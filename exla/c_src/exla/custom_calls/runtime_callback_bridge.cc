...
Result InvokeIoCallback(xla::ffi::Span<const int64_t> callback_id_words,
  ...
  // io_callback never writes output buffers: they are aliased to the input
  // buffers via output_operand_aliases, so we pass an empty outputs list and
  // the Elixir side replies with an empty binary list.
  ...
  if (callback_server_pid_arg.dtype != xla::ffi::DataType::U8) {
    Result res;
    res.ok = false;
    res.error = "io_callback server pid tensor must have dtype u8";
    return res;
  }
  ...
  if (callback_server_pid_arg.dims.size() != 1 ||
    ...
    Result res;
    res.ok = false;
    res.error = "io_callback server pid tensor has unexpected shape";
    return res;
  }
  ...
  if (callback_server_pid_arg.size_bytes != callback_server_pid_size) {
    Result res;
    res.ok = false;
    res.error = "io_callback server pid tensor must contain encoded ErlNifPid bytes";
    return res;
  }
  ...
  if (!enif_binary_to_term(msg_env, id_bytes, callback_id_size,
    ...
    Result res;
    res.ok = false;
    res.error = "failed to decode io_callback id term";
    return res;
  }
  ...
  // Note: uses "exla_io_callback" (not "exla_runtime_call") so the Elixir
  // Outfeed process dispatches to the io_callback handler.
  auto msg = std::make_tuple(fine::Atom("exla_io_callback"),
                             fine::Term(callback_id_term), args_terms, pending);
