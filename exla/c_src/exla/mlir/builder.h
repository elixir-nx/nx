...
class MLIRFunction {
 public:
  ...
  mlir::Value TanOp(mlir::Value operand);
  ...
  std::vector<mlir::Value> TopKOp(mlir::Value operand, int64_t k);
  std::vector<mlir::Value> SortOp(MLIRFunction *comparator, std::vector<mlir::Value> operand, int64_t dim, bool status);
  ...
  std::vector<mlir::Value> InlineCallOp(std::vector<mlir::Value> inputs, MLIRFunction *computation);
  std::pair<std::vector<mlir::Value>, std::pair<mlir::Region *, mlir::Region *>>
  WhileOp(std::vector<mlir::Value> initial);
