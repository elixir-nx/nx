defmodule Nx.Defn.Compiler do
  ...
  defp normalize({:case, meta, [expr, [do: clauses]]}, state) do
    # We don't explicitly allow case, it must be generated by one
    ...
    {clauses, state} =
      Enum.map_reduce(clauses, state, fn {:->, meta, [[condition], expr]}, acc ->
        {expr, acc} = normalize(expr, acc)
        {{:->, meta, [[normalize_transform(condition)], expr]}, acc}
      end)
  ...
  def normalize_transform(transform) do
    Macro.prewalk(transform, fn
