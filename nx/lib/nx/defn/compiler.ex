defmodule Nx.Defn.Compiler do
  ...
  defp normalize({:case, meta, [expr, [do: clauses]]}, state) do
    # We don't explicitly allow case, it must be generated by one of the Kernel macros.
    unless meta[:defn] do
      compile_error!(meta, state, "undefined function case/2 (there is no such import)")
    end

    {expr, state} = normalize(expr, state)

    {clauses, state} =
      Enum.map_reduce(clauses, state, fn {:->, meta, [[pattern], expr]}, acc ->
        # We don't validate patterns and guards because
        # they are validated in the macros that generate case.
        pattern =
        ...
        {expr, acc} = normalize(expr, acc)
        {{:->, meta, [[pattern], expr]}, acc}
      end)

    {{:case, meta, [expr, [do: clauses]]}, state}
  end
