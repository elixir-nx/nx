...
namespace exla {
  ...
  xla::Status CompileComputation(const xla::XlaComputation& computation) {
    ...
    LOG(WARNING) << tensorflow::ReadTextProto(tensorflow::Env::Default(), "/home/sean/projects/exla/priv/proto.pbtext", &config);
    ...
    LOG(WARNING) << tensorflow::WriteStringToFile(tensorflow::Env::Default(), "my_function.o", absl::string_view(obj.data(), obj.size()));

    tensorflow::tfcompile::CodegenOpts codegen_opts;
    codegen_opts.class_name = "my_class";
    codegen_opts.gen_name_to_index = true;
    codegen_opts.gen_program_shape = true;
    codegen_opts.gen_hlo_profile_printer_data = true;
    codegen_opts.target_triple = "x86_64-pc-linux";

    LOG(WARNING) << tensorflow::tfcompile::ParseCppClass("my_class", &codegen_opts.class_name, &codegen_opts.namespaces);
    ...
    LOG(WARNING) << tensorflow::WriteStringToFile(tensorflow::Env::Default(), "my_function.h", header);

    // At this point, we would need to use the compilation result to
    // generate a NIF that reads in arguments (as binaries/lists)
    // We would then need to compile the NIF using Bazel to a shared object
    // because of the TF deps
    return tensorflow::Status::OK();
  }
}